...
class SugarCrmWrapper
{
     ...
     * @return array
     */
    public function getModuleFields($module, $option = false)
    {
        $this->setFormParams([
            'session'     => $this->getSession(),
            'module_name' => $module,
        ...
        return $this->transformResponseField($request);
    }
     ...
     * @return mixed
     */
    public function transformResponseField($data)
    {
        foreach ($data["module_fields"] as $key => $moduleField) {
            $options = [];
            if (count($moduleField['options']) > 0) {
                foreach ($moduleField['options'] as $value) {
                    $options[$value['name']] = $value['value'];
                }
                $data["module_fields"][$key]['options'] = $options;
            }
        }

        return array_values($data["module_fields"]);
    }
